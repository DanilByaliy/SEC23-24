...
app.get('/login', (_, res) => {
    res.redirect(`https://${process.env.AUTH0_DOMAIN}/authorize?response_type=code&client_id=${process.env.CLIENT_ID}&redirect_uri=http://localhost:3000&scope=openid%20profile%20offline_access&audience=${process.env.AUDIENCE}&state=SEC-1233344567890`);
});
...
app.post('/api/loginByCode', async (req, res) => {
    ...
    const response = await authByCode(code);
    if (response.ok) {
        const { access_token, refresh_token, id_token } = await response.json();
        return res.json({ access_token, refresh_token, id_token });
    }
